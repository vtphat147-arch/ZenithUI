...
const Components = () => {
  ...
  return (
    <div className="min-h-screen bg-[#050510] text-white flex flex-col">
      ...
      <section className="relative pt-32 pb-16 overflow-hidden">
        ...
        <div className="container mx-auto px-6 relative z-10 max-w-7xl">
          ...
          >
            ...
            <h2 className="text-4xl md:text-6xl font-black mb-6 bg-gradient-to-r from-white via-slate-200 to-indigo-400 bg-clip-text text-transparent">
              ZenithUI Components
            </h2>
      ...
      <main className="flex-grow py-8 border-t border-white/5 bg-[#070714]">
        <div className="container mx-auto px-6 max-w-7xl">
          ...
          <div className="flex flex-col gap-6 mb-10">
            ...
            <div className="flex flex-col lg:flex-row gap-4 justify-between items-stretch lg:items-center">
              ...
              <div className="relative flex-grow max-w-2xl">
                ...
                <input
                  ...
                  className="w-full pl-11 pr-5 py-3 bg-white/[0.02] border border-white/10 rounded-2xl focus:ring-2 focus:ring-indigo-500/30 outline-none transition-all duration-300 text-white placeholder-slate-400 hover:bg-white/[0.04]"
                />
              </div>

              {/* Inline Selects (Framework & Sort) */}
              <div className="flex gap-3">
                <div className="relative flex items-center bg-white/[0.02] border border-white/10 rounded-2xl px-3 hover:bg-white/[0.04] transition-colors">
                  ...
                  <select
                    ...
                    className="bg-transparent py-3 pr-2 text-xs md:text-sm text-slate-300 outline-none cursor-pointer font-medium"
                  >
            ...
            <div className="flex items-center justify-between pb-4 border-b border-white/5">
              <span className="text-xs md:text-sm text-slate-400 font-medium">
                Hiển thị <span className="text-white font-bold">{components.length}</span> trong số <span className="text-white font-bold">{pagination.total}</span> components
              </span>
          ...
          {loading ? (
            <div className="text-center py-32">
              ...
              <p className="text-slate-400 text-sm">Đang tải components...</p>
            </div>
          ) : components.length === 0 ? (
            <div className="text-center py-24 bg-white/[0.01] border border-white/5 rounded-3xl max-w-4xl mx-auto">
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
              {components.map((component, index) => (
                <motion.div
                  ...
                  className="component-card flex flex-col group border-white/5 h-full"
                >
                  {/* Preview Area */}
                  <Link to={`/components/${component.id}`} className="relative bg-[#0d0d1e] overflow-hidden block">
                    <div className="relative w-full aspect-video bg-gradient-to-br from-white/[0.01] to-white/[0.02]">
                      ...
                      <div className="absolute top-3 right-3 bg-black/80 backdrop-blur-md text-white/80 px-3 py-1 rounded-full text-xs font-semibold border border-white/10 z-10 flex items-center gap-1.5">
                      ...
                      {component.featured && (
                        <div className="absolute top-3 left-3 bg-indigo-500/20 backdrop-blur-md text-indigo-300 px-3 py-1 rounded-full text-xs font-semibold border border-indigo-500/30 z-10 flex items-center gap-1">
                  ...
                  <div className="p-6 flex flex-col flex-grow">
                    <Link to={`/components/${component.id}`} className="inline-block mb-2">
                      <h3 className="text-lg font-bold text-white group-hover:text-indigo-400 transition-colors duration-300 line-clamp-1">
                        {component.name}
                      </h3>
                    </Link>
                    <p className="text-xs md:text-sm text-slate-400 mb-5 line-clamp-2 leading-relaxed flex-grow">
                    ...
                    {component.tags && (
                      <div className="flex flex-wrap gap-1.5 mb-5">
                        {component.tags.slice(0, 3).map((tag, idx) => (
                          <span key={idx} className="text-[10px] uppercase font-bold tracking-wider px-2 py-0.5 bg-white/[0.03] text-slate-400 rounded-md border border-white/5">
                            {tag.trim()}
                          </span>
                        ))}
                      </div>
                    ...
                    <div className="flex items-center justify-between mt-auto pt-4 border-t border-white/5 text-xs text-slate-500">
                      ...
                      {component.framework && (
                        <span className="text-[10px] font-semibold font-mono text-indigo-400 uppercase bg-indigo-500/10 px-2.5 py-0.5 rounded-full border border-indigo-500/10">
                          {component.framework}
                        </span>
                      )}
                    </div>
          ...
          {pagination.totalPages > 1 && (
            <div className="flex items-center justify-center gap-2 mt-16">
              <button
                ...
                className="px-4 py-2 bg-white/[0.02] border border-white/10 rounded-xl disabled:opacity-30 disabled:cursor-not-allowed hover:bg-white/[0.04] transition-colors text-sm text-slate-300 font-semibold"
              >
                Trước
              </button>
